...
def random_walk(bits, sum_distribution, transition_count, transition_probs):
...
def markov_transitions(transition_probs, transition_count):
...
def distribution(sum_distribution):
...
def plot_rand_walk(keys):
    ...
    plt.ylim([-128, 128])
    for key in keys:
        sum_distribution, transition_count, transition_probs = random_walk(key, sum_distribution, transition_count, transition_probs)
    plt.tight_layout()
    distribution(sum_distribution)
    markov_transitions(transition_probs, transition_count)
